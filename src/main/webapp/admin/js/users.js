$(function () {
    $('#data').datagrid({
        ...
        columns:[[
            ...
            {field:'isadmin',title:'身份',width:100},
...
function delSingle(id){
    $.messager.confirm('确认提示框', '确认删除吗？', function(result){
        if (result){
            ...
            $.post("/users/deleteByPrimaryKey",{"id":id},function(data){
...
function DeleteByFruitName(){
    ...
    $.messager.confirm("系统提示", "你确定要删除<font color=red> " + SelectRows.length + " </font>条数据吗?", function(xo){
        if( xo ){
            ...
            $.post("/users/deleteMore",{ids:SelectIndexToString},function(result){
                if( result.result>0 ){
...
function UpdateDialog(){
    ...
    $("#upDialogForm").form('submit',{
        url:"/users/updateByPrimaryKeySelective",
...
function searchUser() {
    ...
    $("#data").datagrid("load",{"telephone":$telephone,"startAge":$startAge,"endAge":$endAge});
